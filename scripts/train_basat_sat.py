"""
BASAT Stage 3 -- SAT-only pilot. Frozen Variant G G1 objective with ONE new ingredient:
SAT-style calibrated, safety-ASYMMETRIC selective filtering of the adversarial cross-entropy
(drop confidence-collapsed / uninformative adversarial CSI windows). NOT full BASAT; NO GAIRAT
weighting, NO MART multiplier, NO TRADES consistency here. This is the WiFi-HAR-domain paper (Pan
et al., SAT, TMC 2024) adapted to the fall-safety objective. Last within-training lever before the
representation-change (BiLSTM) pivot.

Objective (per batch):
    L = base_FWCE_selected + lam_s*L_src + lam_f*L_fall + lam_t*L_tgt   (G1 margin terms UNCHANGED;
        the margins still see ALL adv windows, so SAT only edits the base-CE adv contribution)
where base_FWCE_selected = sum_i (mult_i cw_{y_i} ce_i)/sum_i (mult_i cw_{y_i}),
  mult_i = 1 for CLEAN windows, mult_i = s_i in {0,1} for ADV windows; ==G1 base CE when all s_i==1.

SAT selection (calibrated, asymmetric; SAT's core "drop too-extreme/too-weak" idea):
  temperature tau fit on validation NLL (refit every `recalib` epochs); calibrated p~ = softmax(z/tau).
  confidence range r_i = max(p~_i) - min(p~_i).
  KEEP adv window i:   fall  (y_i=fall):  r_i > alpha_f   (permissive -- keep near-all falls)
                       nonfall (y_i!=f):  r_i > alpha_n   (stricter -- drop diffuse/uninformative)
  s_i = 1 if kept else 0. Burn-in: s_i==1 for the first `burn_in` epochs (cold model -> everything
  looks collapsed; recall ~0 until ~ep27, so select only after some confidence forms).

Scope: seed 42 ONLY, LeNet only, same UT-HAR split, same FGSM/PGD eps=0.030, same clean guard +
selection-v2 as G1. Window-level, digital-domain, white-box; NOT solved/certified/clinical/OTA.
VALIDATION-only for tau/thresholds/go-no-go. NO test set used in training or selection.

Go/no-go bar (validation, best G1 checkpoint): val PGD AUROC of P(fall) > 0.906; recall@FAR<=10% >= 0.477.

Modes: --self-check | --smoke | --pilot   (see Stage 1/2 scripts for the pattern).
Commands:
    python scripts/train_basat_sat.py --setting SA1 --self-check
    python scripts/train_basat_sat.py --setting SA1 --smoke --epochs 18 --smoke-batches 5
    python scripts/train_basat_sat.py --setting SA1 --pilot
"""
from __future__ import annotations
from pathlib import Path
import argparse, csv, json, platform, sys, time
from datetime import datetime, timezone
import numpy as np
import torch
import torch.nn.functional as Fnn


def import_variantG():
    sd = Path(__file__).resolve().parent
    if str(sd) not in sys.path:
        sys.path.insert(0, str(sd))
    import train_variantG_targeted_hardneg as tvg
    return tvg


tvg = import_variantG()
WALK, RUN = tvg.WALK, tvg.RUN
V2_GUARD_ACC, V2_GUARD_F1 = tvg.V2_GUARD_ACC, tvg.V2_GUARD_F1
G1 = tvg.SETTINGS["G1"]
BASE_LAM_S, BASE_LAM_F = tvg.LAM_S, tvg.LAM_F
BASE_LAM_T, BASE_W_WR = G1["lam_t"], G1["w_wr"]
TAU_GRID = np.round(np.linspace(0.5, 5.0, 46), 3)

SETTINGS = {
    "SA1": {"alpha_f": 0.03, "alpha_n": 0.08, "burn_in": 15, "recalib": 10, "desc": "asym confidence-collapse filter"},
    "SA2": {"alpha_f": 0.03, "alpha_n": 0.12, "burn_in": 15, "recalib": 10, "desc": "stricter nonfall (BLOCKED until approved)"},
}
RUNNABLE = {"SA1"}


# --------------------------------------------------------------------------- temperature calibration
def fit_temperature(model, val_loader, device, grid=TAU_GRID):
    """tau minimizing clean validation NLL (grid search on cached logits). CPU-cheap (one val fwd)."""
    model.eval()
    zs, ys = [], []
    with torch.no_grad():
        for x, y in val_loader:
            zs.append(model(x.to(device).float()).float().cpu())
            ys.append(y.long().cpu())
    Z = torch.cat(zs); Y = torch.cat(ys)
    best_tau, best_nll = 1.0, float("inf")
    for tau in grid:
        nll = Fnn.cross_entropy(Z / float(tau), Y).item()
        if nll < best_nll:
            best_nll, best_tau = nll, float(tau)
    return best_tau, best_nll


# --------------------------------------------------------------------------- SAT asymmetric selection
def sat_select(adv_logits, adv_lab, fall_idx, tau, alpha_f, alpha_n):
    """Binary keep-mask s_i in {0,1} over adv windows (calibrated confidence-collapse, asymmetric)."""
    n = adv_lab.numel()
    if n == 0:
        return torch.ones(0, device=adv_logits.device), {"n": 0, "keep": 0, "keep_rate": float("nan"),
                                                          "keep_fall": float("nan"), "keep_nonfall": float("nan")}
    p = torch.softmax(adv_logits.detach() / tau, dim=1)
    r = p.max(dim=1).values - p.min(dim=1).values
    f = adv_lab == fall_idx
    s = torch.ones(n, device=adv_logits.device)
    s[f] = (r[f] > alpha_f).float()
    s[~f] = (r[~f] > alpha_n).float()
    diag = {"n": int(n), "keep": int(s.sum().item()), "keep_rate": float(s.mean().item()),
            "keep_fall": float(s[f].mean().item()) if f.any() else float("nan"),
            "keep_nonfall": float(s[~f].mean().item()) if (~f).any() else float("nan")}
    return s, diag


def reweighted_base_loss(outputs, batch_y, n_clean, adv_mult, class_weights):
    """sum mult*cw*ce / sum mult*cw; mult=1 clean, s_i adv. ==G1 base CE when all adv mult==1.
    If ALL adv windows are dropped, falls back to clean-only CE (denominator still > 0 from clean)."""
    ce = Fnn.cross_entropy(outputs, batch_y, reduction="none")
    cw = class_weights[batch_y]
    mult = torch.ones_like(cw)
    if adv_mult.numel() > 0:
        mult[n_clean:] = adv_mult
    den = (mult * cw).sum().clamp_min(1e-12)
    return (mult * cw * ce).sum() / den


def train_one_epoch_sat(model, loader, class_weights, atk_criterion, optimizer, device,
                        train_epsilons, train_pgd_steps, rng, tsg, cfg, fall_idx, epoch, tau,
                        max_batches=None):
    model.train()
    use_sel = epoch > cfg["burn_in"]
    tot_loss = tot = 0.0
    s = {"base": 0.0, "src": 0.0, "fall": 0.0, "tgt": 0.0}
    kd = {"rate": 0.0, "kf": 0.0, "kn": 0.0, "nb": 0}
    nb = 0
    for bi, (inputs, labels) in enumerate(loader):
        if max_batches is not None and bi >= max_batches:
            break
        inputs = inputs.to(device).float()
        labels = labels.type(torch.LongTensor).to(device)
        bs = labels.size(0)
        eps = float(rng.choice(train_epsilons)); pgd_alpha = eps / 4.0
        n_clean = bs // 2; n_fgsm = (bs - n_clean) // 2
        clean_x = inputs[:n_clean]
        fgsm_src, fgsm_lbl = inputs[n_clean:n_clean + n_fgsm], labels[n_clean:n_clean + n_fgsm]
        pgd_src, pgd_lbl = inputs[n_clean + n_fgsm:], labels[n_clean + n_fgsm:]

        model.eval()
        fgsm_adv = tsg.fgsm_perturb(model, fgsm_src, fgsm_lbl, atk_criterion, eps)
        pgd_adv = tsg.pgd_perturb(model, pgd_src, pgd_lbl, atk_criterion, eps, train_pgd_steps, pgd_alpha)
        nf_mask = labels != fall_idx
        if BASE_LAM_T > 0 and nf_mask.any():
            tgt_adv = tvg.targeted_fall_pgd(model, inputs[nf_mask], fall_idx, atk_criterion, eps, train_pgd_steps, pgd_alpha)
            tgt_lab = labels[nf_mask]
        else:
            tgt_adv, tgt_lab = None, labels[:0]
        model.train()

        batch_x = torch.cat([clean_x, fgsm_adv, pgd_adv], dim=0)
        batch_y = labels
        n_clean = clean_x.size(0)

        optimizer.zero_grad()
        outputs = model(batch_x).float()
        adv_out, adv_lab = outputs[n_clean:], batch_y[n_clean:]
        if use_sel:
            sel, d = sat_select(adv_out, adv_lab, fall_idx, tau, cfg["alpha_f"], cfg["alpha_n"])
            kd["rate"] += d["keep_rate"]; kd["kf"] += (d["keep_fall"] if np.isfinite(d["keep_fall"]) else 1.0)
            kd["kn"] += (d["keep_nonfall"] if np.isfinite(d["keep_nonfall"]) else 1.0); kd["nb"] += 1
        else:
            sel = torch.ones(adv_lab.numel(), device=device)
        base = reweighted_base_loss(outputs, batch_y, n_clean, sel, class_weights)
        tgt_out = model(tgt_adv).float() if (tgt_adv is not None and tgt_adv.numel() > 0) else None
        L_src, L_fall, L_tgt = tvg.variantG_margin_terms(adv_out, adv_lab, tgt_out, tgt_lab, BASE_W_WR, fall_idx, device)
        loss = base + BASE_LAM_S * L_src + BASE_LAM_F * L_fall + BASE_LAM_T * L_tgt
        loss.backward(); optimizer.step()

        tot_loss += loss.item() * bs; tot += bs; nb += 1
        s["base"] += float(base.item()); s["src"] += float(L_src.item())
        s["fall"] += float(L_fall.item()); s["tgt"] += float(L_tgt.item())
    d = max(1, nb); dk = max(1, kd["nb"])
    return {"train_loss": tot_loss / max(1, tot), "mean_base": s["base"] / d, "mean_src_motion": s["src"] / d,
            "mean_fall_margin": s["fall"] / d, "mean_targeted": s["tgt"] / d, "select_active": int(use_sel),
            "keep_rate": kd["rate"] / dk, "keep_fall": kd["kf"] / dk, "keep_nonfall": kd["kn"] / dk, "batches": nb}


def parse_args():
    p = argparse.ArgumentParser(description="BASAT Stage 3 -- SAT-only (G1 + asymmetric adv selection).")
    p.add_argument("--setting", choices=sorted(SETTINGS.keys()), default="SA1")
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--epochs", type=int, default=70)
    p.add_argument("--patience", type=int, default=15)
    p.add_argument("--min-epochs", type=int, default=35)
    p.add_argument("--lr", type=float, default=1e-3)
    p.add_argument("--batch-size", type=int, default=64)
    p.add_argument("--train-pgd-steps", type=int, default=7)
    p.add_argument("--fall-weight", type=float, default=3.0)
    p.add_argument("--self-check", action="store_true")
    p.add_argument("--smoke", action="store_true")
    p.add_argument("--smoke-batches", type=int, default=5)
    p.add_argument("--pilot", action="store_true")
    return p.parse_args()


def _class_weights(F, fall_weight):
    cw = torch.ones(F["num_classes"], device=F["device"]); cw[F["fall_idx"]] = fall_weight
    return cw


def run_self_check(args, F):
    cfg = SETTINGS[args.setting]; device = F["device"]; fall_idx = F["fall_idx"]
    print("=" * 78); print(f"BASAT Stage 3 SAT --self-check  setting={args.setting} ({cfg['desc']})")
    assert fall_idx == 1 and F["num_classes"] == 7 and (WALK, RUN) == (2, 4)
    print(f"  class-index assertions PASSED: FALL={fall_idx} WALK={WALK} RUN={RUN}")
    sc, ok = tvg.targeted_sign_check(F["model"], F["train_loader"], F["atk_criterion"], fall_idx, device)
    print(f"  targeted-PGD sign check: increased={sc['increased']}")
    if not ok:
        raise SystemExit("SIGN CHECK FAILED.")
    # temperature fit
    tau, nll = fit_temperature(F["model"], F["val_loader"], device)
    print(f"  temperature fit on val NLL: tau={tau:.3f} (val NLL={nll:.4f})")
    assert 0.5 <= tau <= 5.0
    # selection drops collapsed, keeps confident; asymmetric (fall more permissive)
    # synthetic: 2 fall (one confident, one near-uniform), 2 nonfall (one confident, one near-uniform)
    z = torch.zeros(4, 7, device=device)
    z[0, fall_idx] = 6.0          # confident fall  -> keep
    z[1] = 0.001 * torch.arange(7, device=device)  # near-uniform fall -> drop (r tiny)
    z[2, 2] = 6.0                 # confident walk  -> keep
    z[3] = 0.001 * torch.arange(7, device=device)  # near-uniform walk -> drop
    lab = torch.tensor([fall_idx, fall_idx, 2, 2], device=device)
    sel, d = sat_select(z, lab, fall_idx, 1.0, cfg["alpha_f"], cfg["alpha_n"])
    print(f"  selection (confident-fall, uniform-fall, confident-walk, uniform-walk): {[int(x) for x in sel.tolist()]}  keep_rate={d['keep_rate']:.2f}")
    assert sel[0] == 1 and sel[2] == 1, "confident windows must be kept"
    assert sel[1] == 0 and sel[3] == 0, "near-uniform (collapsed) windows must be dropped"
    assert cfg["alpha_f"] <= cfg["alpha_n"], "fall threshold must be <= nonfall (more permissive on falls)"
    print("  collapse-drop + asymmetry PASSED")
    # reduces-to-G1 when all kept
    F["model"].train()
    inputs, labels = next(iter(F["train_loader"]))
    inputs = inputs.to(device).float(); labels = labels.type(torch.LongTensor).to(device)
    bs = labels.size(0); n_clean = bs // 2; n_fgsm = (bs - n_clean) // 2
    eps = float(F["rng"].choice(F["train_epsilons"])); a = eps / 4.0
    fgsm = F["tsg"].fgsm_perturb(F["model"], inputs[n_clean:n_clean + n_fgsm], labels[n_clean:n_clean + n_fgsm], F["atk_criterion"], eps)
    pgd = F["tsg"].pgd_perturb(F["model"], inputs[n_clean + n_fgsm:], labels[n_clean + n_fgsm:], F["atk_criterion"], eps, args.train_pgd_steps, a)
    outputs = F["model"](torch.cat([inputs[:n_clean], fgsm, pgd], 0)).float()
    cw = _class_weights(F, args.fall_weight); n_adv = labels.numel() - n_clean
    base_keepall = reweighted_base_loss(outputs, labels, n_clean, torch.ones(n_adv, device=device), cw)
    g1_base = torch.nn.CrossEntropyLoss(weight=cw)(outputs, labels)
    print(f"  reduces-to-G1 when all kept: sel-base={base_keepall.item():.6f}  G1_CE={g1_base.item():.6f}  |diff|={abs(base_keepall.item()-g1_base.item()):.2e}")
    assert abs(base_keepall.item() - g1_base.item()) < 1e-5
    # finite full loss with real selection
    adv_out, adv_lab = outputs[n_clean:], labels[n_clean:]
    sel, dd = sat_select(adv_out, adv_lab, fall_idx, tau, cfg["alpha_f"], cfg["alpha_n"])
    base = reweighted_base_loss(outputs, labels, n_clean, sel, cw)
    nf = labels != fall_idx
    tgt_out = F["model"](tvg.targeted_fall_pgd(F["model"], inputs[nf], fall_idx, F["atk_criterion"], eps, args.train_pgd_steps, a)).float()
    L_src, L_fall, L_tgt = tvg.variantG_margin_terms(adv_out, adv_lab, tgt_out, labels[nf], BASE_W_WR, fall_idx, device)
    total = base + L_src + L_fall + L_tgt
    print(f"  one-batch keep_rate={dd['keep_rate']:.2f} (fall={dd['keep_fall']:.2f} nonfall={dd['keep_nonfall']:.2f}); total={total.item():.4f}")
    for v in (base, L_src, L_fall, L_tgt, total):
        assert np.isfinite(v.item())
    assert L_tgt.item() > 0
    print("  PASS: sign check, tau fit, collapse-drop+asymmetry, reduces-to-G1, finite losses.")
    print("=" * 78)


def run_smoke(args, F):
    cfg = SETTINGS[args.setting]; tsg, s1 = F["tsg"], F["s1"]; cw = _class_weights(F, args.fall_weight)
    run = f"seed{args.seed}_basat_sat_{args.setting}_smoke"
    base = F["exp"] / "results" / "safety_guided_defense" / "boundary_aware_selective_at" / "sat" / "_smoke" / f"seed{args.seed}"
    base.mkdir(parents=True, exist_ok=True)
    print("=" * 78); print(f"BASAT Stage 3 SAT --smoke  setting={args.setting}  run={run}")
    print(f"  alpha_f={cfg['alpha_f']} alpha_n={cfg['alpha_n']} burn_in={cfg['burn_in']} recalib={cfg['recalib']}")
    history = []; start = time.time(); tau = 1.0
    for epoch in range(1, args.epochs + 1):
        if epoch > cfg["burn_in"] and (epoch == cfg["burn_in"] + 1 or (epoch - cfg["burn_in"] - 1) % cfg["recalib"] == 0):
            tau, _ = fit_temperature(F["model"], F["val_loader"], F["device"])
        tr = train_one_epoch_sat(F["model"], F["train_loader"], cw, F["atk_criterion"], F["optimizer"], F["device"],
                                 F["train_epsilons"], args.train_pgd_steps, F["rng"], tsg, cfg, F["fall_idx"], epoch, tau,
                                 max_batches=args.smoke_batches)
        vb = tsg.compute_validation_bundle(s1, F["model"], F["val_loader"], F["atk_criterion"], F["device"])
        for k in ("train_loss", "mean_base", "mean_src_motion", "mean_fall_margin", "mean_targeted"):
            assert np.isfinite(tr[k]), f"{k} not finite at epoch {epoch}"
        history.append({"epoch": epoch, "tau": tau, **{k: tr[k] for k in tr if k != "batches"},
                        "val_clean_accuracy": vb["val_clean_accuracy"], "val_clean_fall_recall": vb["val_clean_fall_recall"],
                        "val_pgd_fall_recall": vb["val_pgd_fall_recall"], "val_pgd_false_fall_alarms": vb["val_pgd_false_fall_alarms"]})
        print(f"  epoch {epoch}/{args.epochs} | loss={tr['train_loss']:.3f} base={tr['mean_base']:.3f} "
              f"sel={tr['select_active']} tau={tau:.2f} keep={tr['keep_rate']:.2f}(f={tr['keep_fall']:.2f}/nf={tr['keep_nonfall']:.2f}) "
              f"| acc={vb['val_clean_accuracy']:.3f} cFR={vb['val_clean_fall_recall']:.3f} pgd_fr={vb['val_pgd_fall_recall']:.3f}")
    elapsed = time.time() - start
    with (base / f"{run}_log.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(history[0].keys())); w.writeheader()
        for r in history:
            w.writerow({k: (f"{r[k]:.6f}" if isinstance(r[k], float) else r[k]) for k in history[0]})
    summary = {"timestamp_utc": datetime.now(timezone.utc).isoformat(), "stage": "basat_sat_smoke",
               "setting": args.setting, "cfg": cfg, "seed": args.seed, "epochs": args.epochs,
               "smoke_batches": args.smoke_batches, "elapsed_seconds": elapsed,
               "per_epoch_seconds_estimate": elapsed / max(1, args.epochs), "final_tau": tau, "test_set_used": False,
               "device": str(F["device"]), "python_version": platform.python_version(), "torch_version": torch.__version__,
               "git_commit": s1.get_command_output(["git", "rev-parse", "HEAD"], cwd=str(F["exp"])),
               "note": "SMOKE ONLY -- code-correctness + runtime + keep-rate sanity; selection off during burn_in; no .pt."}
    with (base / f"{run}_summary.json").open("w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)
    print(f"  smoke done in {elapsed:.1f}s ({summary['per_epoch_seconds_estimate']:.2f}s/epoch). final tau={tau:.2f}")
    print("=" * 78)
    return summary


def run_pilot(args, F):
    assert args.seed == 42, "pilot is seed 42 ONLY"
    cfg = SETTINGS[args.setting]; tsg, s1 = F["tsg"], F["s1"]; device = F["device"]; cw = _class_weights(F, args.fall_weight)
    run = f"seed{args.seed}_basat_sat_{args.setting}"
    base = F["exp"] / "results" / "safety_guided_defense" / "boundary_aware_selective_at" / "sat" / f"seed{args.seed}" / args.setting
    ck_dir = F["exp"] / "checkpoints" / "safety_guided_defense" / "boundary_aware_selective_at" / "sat" / f"seed{args.seed}" / args.setting
    logs_dir = base / "logs"; ana_dir = base / "analysis"; meta_dir = base / "metadata"
    for d in (ck_dir, logs_dir, ana_dir, meta_dir):
        d.mkdir(parents=True, exist_ok=True)
    ck = {k: ck_dir / f"{run}_{k}_best.pt" for k in ("v2safety", "v2maxrec", "v2lowFA", "v2macroF1")}
    ck_last = ck_dir / f"{run}_last.pt"

    print("=" * 78); print(f"BASAT Stage 3 SAT FULL PILOT  setting={args.setting} ({cfg['desc']})  run={run}")
    print(f"  alpha_f={cfg['alpha_f']} alpha_n={cfg['alpha_n']} burn_in={cfg['burn_in']} recalib={cfg['recalib']} fw={args.fall_weight}")
    print(f"  guard acc>={V2_GUARD_ACC} & mF1>={V2_GUARD_F1} | epochs={args.epochs} patience={args.patience} min={args.min_epochs}")
    print("=" * 78)

    history = []
    best = {"v2safety": (-1e9, -1), "v2maxrec": ((-1e9, 1e9), -1), "v2lowFA": ((1e9, -1e9), -1), "v2macroF1": (-1e9, -1)}
    best_rec = {}; no_improve = 0; start = time.time(); sel_ever = False; tau = 1.0

    for epoch in range(1, args.epochs + 1):
        if epoch > cfg["burn_in"] and (epoch == cfg["burn_in"] + 1 or (epoch - cfg["burn_in"] - 1) % cfg["recalib"] == 0):
            tau, _ = fit_temperature(F["model"], F["val_loader"], device)
        tr = train_one_epoch_sat(F["model"], F["train_loader"], cw, F["atk_criterion"], F["optimizer"], device,
                                 F["train_epsilons"], args.train_pgd_steps, F["rng"], tsg, cfg, F["fall_idx"], epoch, tau)
        for k in ("train_loss", "mean_base", "mean_src_motion", "mean_fall_margin", "mean_targeted"):
            if not np.isfinite(tr[k]):
                raise SystemExit(f"STOP (numerical): {k} not finite at epoch {epoch}.")
        sel_ever = sel_ever or bool(tr["select_active"])
        vb = tsg.compute_validation_bundle(s1, F["model"], F["val_loader"], F["atk_criterion"], device)
        sc = tvg.safety_score(vb)
        acc = vb["val_clean_accuracy"]; f1 = vb["val_clean_macro_f1"]
        rec = vb["val_pgd_fall_recall"]; fpv = vb["val_pgd_false_fall_alarms"]
        eligible = (acc >= V2_GUARD_ACC and f1 >= V2_GUARD_F1)

        def save(key):
            torch.save({"epoch": epoch, "model_state_dict": F["model"].state_dict(), "selection_method": key,
                        "val_bundle": vb, "safety_score": sc, "run_name": run, "setting": args.setting, "tau": tau,
                        "cfg": cfg, "args": vars(args)}, ck[key])

        flags = {k: 0 for k in ck}
        if eligible:
            if sc > best["v2safety"][0]:
                best["v2safety"] = (sc, epoch); best_rec["v2safety"] = vb; save("v2safety"); flags["v2safety"] = 1; no_improve = 0
            if (rec, -fpv) > best["v2maxrec"][0]:
                best["v2maxrec"] = ((rec, -fpv), epoch); best_rec["v2maxrec"] = vb; save("v2maxrec"); flags["v2maxrec"] = 1
            if rec >= tvg.LOWFA_RECALL_FLOOR and (fpv, -rec) < best["v2lowFA"][0]:
                best["v2lowFA"] = ((fpv, -rec), epoch); best_rec["v2lowFA"] = vb; save("v2lowFA"); flags["v2lowFA"] = 1
        if not flags["v2safety"] and best["v2safety"][1] > 0:
            no_improve += 1
        if f1 > best["v2macroF1"][0]:
            best["v2macroF1"] = (f1, epoch); best_rec["v2macroF1"] = vb; save("v2macroF1"); flags["v2macroF1"] = 1

        history.append({"epoch": epoch, "tau": tau, "train_loss": tr["train_loss"], "mean_base": tr["mean_base"],
                        "mean_src_motion": tr["mean_src_motion"], "mean_fall_margin": tr["mean_fall_margin"],
                        "mean_targeted": tr["mean_targeted"], "select_active": tr["select_active"], "keep_rate": tr["keep_rate"],
                        "keep_fall": tr["keep_fall"], "keep_nonfall": tr["keep_nonfall"],
                        "val_clean_accuracy": acc, "val_clean_macro_f1": f1, "val_clean_fall_recall": vb["val_clean_fall_recall"],
                        "val_fgsm_fall_recall": vb["val_fgsm_fall_recall"], "val_pgd_fall_recall": rec,
                        "val_fgsm_false_fall_alarms": vb["val_fgsm_false_fall_alarms"], "val_pgd_false_fall_alarms": fpv,
                        "val_normalized_false_alarm_burden": vb["val_normalized_false_alarm_burden"], "safety_score": sc,
                        "v2_eligible": int(eligible), "sel_v2safety": flags["v2safety"], "sel_v2maxrec": flags["v2maxrec"],
                        "sel_v2lowFA": flags["v2lowFA"], "sel_v2macroF1": flags["v2macroF1"]})
        print(f"Epoch {epoch:03d}/{args.epochs} | loss={tr['train_loss']:.3f} base={tr['mean_base']:.3f} "
              f"sel={tr['select_active']} tau={tau:.2f} keep={tr['keep_rate']:.2f}(f{tr['keep_fall']:.2f}/n{tr['keep_nonfall']:.2f}) | "
              f"acc={acc:.3f} f1={f1:.3f} cFR={vb['val_clean_fall_recall']:.3f} pgd_fr={rec:.3f} pgd_FP={fpv} "
              f"score={sc:.3f} v2elig={int(eligible)} "
              f"[{'S' if flags['v2safety'] else '.'}{'R' if flags['v2maxrec'] else '.'}"
              f"{'L' if flags['v2lowFA'] else '.'}{'F' if flags['v2macroF1'] else '.'}]")
        if args.patience > 0 and epoch >= args.min_epochs and no_improve >= args.patience:
            print(f"Early stopping at epoch {epoch} (best v2safety epoch {best['v2safety'][1]})."); break

    if not sel_ever:
        raise SystemExit("STOP: SAT selection never activated (burn_in >= epochs run?).")
    elapsed = time.time() - start
    last_epoch = history[-1]["epoch"]
    torch.save({"epoch": last_epoch, "model_state_dict": F["model"].state_dict(), "selection_method": "last",
                "run_name": run, "setting": args.setting, "cfg": cfg, "args": vars(args)}, ck_last)
    fields = list(history[0].keys())
    with (logs_dir / f"{run}_training_log.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields); w.writeheader()
        for r in history:
            w.writerow({k: (f"{r[k]:.6f}" if isinstance(r[k], float) else r[k]) for k in fields})
    with (ana_dir / f"{run}_selection_candidates.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["selection", "selected_epoch", "val_clean_acc", "val_clean_macro_f1", "val_pgd_recall", "val_pgd_false_alarms", "safety_score"])
        for k in ck:
            ep = best[k][1]; vbk = best_rec.get(k, {})
            w.writerow([k, ep, f"{vbk.get('val_clean_accuracy', float('nan')):.4f}", f"{vbk.get('val_clean_macro_f1', float('nan')):.4f}",
                        f"{vbk.get('val_pgd_fall_recall', float('nan')):.4f}", vbk.get('val_pgd_false_fall_alarms', ''),
                        f"{tvg.safety_score(vbk):.4f}" if vbk else ""])
    meta = {"timestamp_utc": datetime.now(timezone.utc).isoformat(), "stage": f"basat_sat_{args.setting}_seed{args.seed}_pilot",
            "method": "BASAT Stage 3 SAT-only (G1 + calibrated asymmetric adv selection)", "namespace": "boundary_aware_selective_at/sat",
            "run_name": run, "setting": args.setting, "cfg": cfg, "seed": args.seed, "fall_weight": args.fall_weight, "final_tau": tau,
            "lam_s": BASE_LAM_S, "lam_f": BASE_LAM_F, "lam_t": BASE_LAM_T, "w_wr": BASE_W_WR,
            "objective": "selected base FWCE (mult=1 clean, s_i in {0,1} adv via calibrated asym confidence-collapse; ==G1 when all kept) "
                         "+ lam_s*L_src + lam_f*L_fall + lam_t*L_tgt; tau fit on val NLL, refit every recalib epochs; burn_in",
            "v2_guard": {"min_clean_accuracy": V2_GUARD_ACC, "min_clean_macro_f1": V2_GUARD_F1},
            "train_epsilons": F["train_epsilons"], "train_pgd_steps": args.train_pgd_steps,
            "epochs_run": last_epoch, "split_sizes": F["split_sizes"], "test_set_used": False,
            "selected_epochs": {k: best[k][1] for k in ck}, "checkpoints": {k: str(v) for k, v in ck.items()},
            "go_no_go_ref": "results/safety_guided_defense/boundary_aware_selective_at/seed42/g1_baseline_val_frontier.json",
            "claim_boundary": "window-level digital-domain white-box; seed42/LeNet only; not solved/certified/clinical/OTA",
            "device": str(device), "python_version": platform.python_version(), "torch_version": torch.__version__,
            "git_commit": s1.get_command_output(["git", "rev-parse", "HEAD"], cwd=str(F["exp"])), "elapsed_seconds": elapsed}
    with (meta_dir / f"{run}_metadata.json").open("w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2)
    print("-" * 78)
    print(f"Done in {elapsed:.1f}s ({last_epoch} epochs). selected epochs: " + ", ".join(f"{k}={best[k][1]}" for k in ck))
    print("  NEXT: export val PGD probabilities for selected checkpoints; compute val PGD AUROC vs the 0.906 bar.")
    print("=" * 78)


def main():
    args = parse_args()
    if args.seed != 42:
        raise SystemExit(f"SAT Stage 3 is seed-42 ONLY for now (got seed {args.seed}).")
    if args.setting not in RUNNABLE and (args.pilot or args.smoke):
        raise SystemExit(f"Only {sorted(RUNNABLE)} is approved to run; {args.setting} is defined but blocked.")
    F = tvg.load_foundation(args)
    if args.self_check:
        run_self_check(args, F); return
    if args.smoke:
        _, ok = tvg.targeted_sign_check(F["model"], F["train_loader"], F["atk_criterion"], F["fall_idx"], F["device"])
        if not ok:
            raise SystemExit("SIGN CHECK FAILED before smoke.")
        run_smoke(args, F); return
    if args.pilot:
        _, ok = tvg.targeted_sign_check(F["model"], F["train_loader"], F["atk_criterion"], F["fall_idx"], F["device"])
        if not ok:
            raise SystemExit("SIGN CHECK FAILED before pilot.")
        run_pilot(args, F); return
    raise SystemExit("Gated: pass --self-check, --smoke, or --pilot.")


if __name__ == "__main__":
    main()
